...
def sniff_limit() -> int:
    try:
        return int(os.getenv("PROXY_SNIFF_BYTES", "1048576"))
    except Exception:
        return 1048576
...
async def sniff_model_and_stream(request: Request) -> Tuple[str, AsyncIterator[bytes]]:
    """
    Возвращает (model, body_stream).
    Не буферизует тело целиком: читает только до PROXY_SNIFF_BYTES, чтобы вытащить model,
    затем отдаёт уже прочитанные чанки + остаток как поток.
    """
    # 1) query param (если кто-то так шлёт)
